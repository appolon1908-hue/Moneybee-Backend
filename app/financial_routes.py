...
async def create_account(payload: LedgerAccountCreate, db: Db, user: User):
    ...
    if existing:
        raise HTTPException(status_code=409, detail={"code": "ACCOUNT_CODE_EXISTS", "message": "Ledger account code already exists."})
    item = LedgerAccount(
...
async def create_period(payload: AccountingPeriodCreate, db: Db, user: User):
    ...
    if overlap:
        raise HTTPException(status_code=409, detail={"code": "ACCOUNTING_PERIOD_OVERLAP", "message": "Accounting periods may not overlap."})
...
async def close_period(period_id: uuid.UUID, db: Db, user: User):
    ...
    if period.status != "OPEN":
        raise HTTPException(status_code=409, detail={"code": "PERIOD_NOT_OPEN", "message": "Only an open accounting period can be closed."})
...
async def create_journal_entry(payload: JournalEntryCreate, db: Db, user: User):
    entry, _postings = await post_journal(db, user, payload)
    return entry
...
async def get_trial_balance(
    ...
    return await trial_balance(db, user, organization_id, as_of)
