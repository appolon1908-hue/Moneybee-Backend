...
from sqlalchemy import CheckConstraint, DateTime, ForeignKey, JSON, Numeric, String, Text, UniqueConstraint, Uuid
...
class LedgerAccount(Base, Record):
    ...
    __table_args__ = (
        UniqueConstraint("organization_id", "code", name="uq_ledger_account_org_code"),
        CheckConstraint(
...
class AccountingPeriod(Base, Record):
    ...
    __table_args__ = (
        UniqueConstraint("organization_id", "name", name="uq_accounting_period_org_name"),
        CheckConstraint("status IN ('OPEN','CLOSED','LOCKED')", name="ck_accounting_period_status"),
        CheckConstraint("ends_at > starts_at", name="ck_accounting_period_range"),
    )
...
class JournalEntry(Base, Record):
    ...
    __table_args__ = (
        ...
        CheckConstraint("status IN ('POSTED','VOID')", name="ck_journal_entry_status"),
    )

    organization_id: Mapped[uuid.UUID] = mapped_column(ForeignKey("organizations.id"), index=True)
    period_id: Mapped[uuid.UUID | None] = mapped_column(ForeignKey("accounting_periods.id"), nullable=True, index=True)
    ...
    reversal_of_id: Mapped[uuid.UUID | None] = mapped_column(ForeignKey("journal_entries.id"), nullable=True)
...
class JournalPosting(Base, Record):
    ...
    __table_args__ = (
        CheckConstraint("side IN ('DEBIT','CREDIT')", name="ck_journal_posting_side"),
        CheckConstraint("amount > 0", name="ck_journal_posting_amount"),
    )

    journal_entry_id: Mapped[uuid.UUID] = mapped_column(ForeignKey("journal_entries.id", ondelete="CASCADE"), index=True)
    account_id: Mapped[uuid.UUID] = mapped_column(ForeignKey("ledger_accounts.id"), index=True)
    ...
    funding_id: Mapped[uuid.UUID | None] = mapped_column(ForeignKey("fundings.id"), nullable=True, index=True)
    commission_id: Mapped[uuid.UUID | None] = mapped_column(ForeignKey("commissions.id"), nullable=True, index=True)
    bank_transaction_id: Mapped[uuid.UUID | None] = mapped_column(ForeignKey("bank_transactions.id"), nullable=True, index=True)
...
class FinancialSettlement(Base, Record):
    ...
    __table_args__ = (
        ...
        CheckConstraint("amount >= 0", name="ck_financial_settlement_amount"),
    )
    ...
    funding_id: Mapped[uuid.UUID | None] = mapped_column(ForeignKey("fundings.id"), nullable=True, index=True)
    commission_id: Mapped[uuid.UUID | None] = mapped_column(ForeignKey("commissions.id"), nullable=True, index=True)
    journal_entry_id: Mapped[uuid.UUID | None] = mapped_column(ForeignKey("journal_entries.id"), nullable=True, index=True)
    amount: Mapped[Decimal] = mapped_column(Numeric(20, 2))
    currency: Mapped[str] = mapped_column(String(3), default="USD")
